...
sub createTargetWeb {
    ...
    my $targetWebObj =
      new Foswiki::Meta( $this->{session}, $this->{targetWeb} );
    $targetWebObj->populateNewWeb(
        $this->{templateWeb},
        {
            WEBSUMMARY => $webSummary,
            WEBBGCOLOR => Foswiki::Func::getPreferencesValue(
                "WEBBGCOLOR", $this->{sourceWeb}
            ),
            BLOGIMAGES => Foswiki::Func::getPreferencesValue( "BLOGIMAGES",
                $this->{sourceWeb} )
              || '%PUBURLPATH%/%WEB%/BlogImages',
            IMAGEALBUM => Foswiki::Func::getPreferencesValue( "IMAGEALBUM",
                $this->{sourceWeb} )
              || '%WEB%/BlogImages',
            NOAUTOLINK => Foswiki::Func::getPreferencesValue( "NOAUTOLINK",
                $this->{sourceWeb} )
              || 'on',
            SITEMAPLIST => Foswiki::Func::getPreferencesValue( "SITEMAPLIST",
                $this->{sourceWeb} )
              || 'on',
        }
    );
...
sub convertCategories {
    ...
    while ( $matches->hasNext ) {
        ...
        my ( $web, $topic ) =
          Foswiki::Func::normalizeWebTopicName( $this->{sourceWeb}, $webTopic );
...
sub convertCategory {
    ...
    my $newTopic =
      new Foswiki::Meta( $this->{session}, $this->{targetWeb}, $newTopicName,
        $newText );

    $newTopic->putKeyed( 'FORM',
        { name => "Applications.ClassificationApp.Category" } );
...
sub copyBlogImages {
    ...
    return
      unless Foswiki::Func::topicExists( $this->{sourceWeb}, "BlogImages" );
    my $sourceObj =
      new Foswiki::Meta( $this->{session}, $this->{sourceWeb}, "BlogImages" );
    my $targetObj = new Foswiki::Meta(
        $this->{session}, $this->{targetWeb},
        "BlogImages",     "---+!! %TOPIC%\n"
    );
    $targetObj->save;

    print STDERR
      "### copying BlogImages from $this->{sourceWeb} to $this->{targetWeb}\n";
    $sourceObj->load();
...
sub convertBlogComments {
    ...
    while ( $matches->hasNext ) {
        ...
        if ($number) {
            $this->{nameOfComment}{$topic} =
              convertCommentNumberToName( $number->{value} );
        }
    }
...
#print STDERR "### ...baseRef=$baseRef\n";
    ...
    $author =
      Foswiki::Func::getWikiName( $author ? $author->{value} : $revAuthor );
...
sub convertBlogEntry {
    ...
    my $newTopic =
      new Foswiki::Meta( $this->{session}, $this->{targetWeb}, $topic,
        $newText );
    ...
    my $tags =
      join( ", ",
        split( /[\s,]+/, $meta->get( "FIELD", "BlogTag" )->{value} ) );
    ...
    my $categories = join(
        ", ",
        map( $_ =
              ( defined $this->{renamedCategories}{$_} )
            ? $this->{renamedCategories}{$_}
            : $_,
            split(
                /\s*,\s*/, $meta->get( "FIELD", "SubjectCategory" )->{value}
              ) )
    );
    ...
    foreach my $attachment ( $meta->find('FILEATTACHMENT') ) {
        print STDERR "### copying attachment $attachment->{name}\n";
        $meta->copyAttachment( $attachment->{name}, $newTopic,
            user => $author );
    }
}
